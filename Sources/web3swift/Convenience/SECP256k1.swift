...
extension SECP256K1 {
    static let context = secp256k1_2_context_create(UInt32(SECP256K1_2_CONTEXT_SIGN|SECP256K1_2_CONTEXT_VERIFY))
    ...
    public static func combineSerializedPublicKeys(keys: [Data], outputCompressed: Bool = false) -> Data? {
        ...
        var storage = ContiguousArray<secp256k1_2_pubkey>()
        let arrayOfPointers = UnsafeMutablePointer< UnsafePointer<secp256k1_2_pubkey>? >.allocate(capacity: numToCombine)
        defer {
        ...
        var publicKey: secp256k1_2_pubkey = secp256k1_2_pubkey()
        let result = withUnsafeMutablePointer(to: &publicKey) { (pubKeyPtr: UnsafeMutablePointer<secp256k1_2_pubkey>) -> Int32 in
            let res = secp256k1_2_ec_pubkey_combine(context!, pubKeyPtr, immutablePointer, numToCombine)
            return res
        }
        if result == 0 {
            return nil
    ...
    internal static func recoverPublicKey(hash: Data, recoverableSignature: inout secp256k1_2_ecdsa_recoverable_signature) -> secp256k1_2_pubkey? {
        ...
        let result = hash.withUnsafeBytes({ (hashRawBufferPointer: UnsafeRawBufferPointer) -> Int32? in
            if let hashRawPointer = hashRawBufferPointer.baseAddress, hashRawBufferPointer.count > 0 {
                ...
                return withUnsafePointer(to: &recoverableSignature, { (signaturePointer:UnsafePointer<secp256k1_2_ecdsa_recoverable_signature>) -> Int32 in
                    withUnsafeMutablePointer(to: &publicKey, { (pubKeyPtr: UnsafeMutablePointer<secp256k1_2_pubkey>) -> Int32 in
                        let res = secp256k1_2_ecdsa_recover(context!, pubKeyPtr,
    ...
    internal static func privateKeyToPublicKey(privateKey: Data) -> secp256k1_2_pubkey? {
        if (privateKey.count != 32) {return nil}
        var publicKey = secp256k1_2_pubkey()
        let result = privateKey.withUnsafeBytes { (pkRawBufferPointer: UnsafeRawBufferPointer) -> Int32? in
            if let pkRawPointer = pkRawBufferPointer.baseAddress, pkRawBufferPointer.count > 0 {
                ...
                let res = secp256k1_2_ec_pubkey_create(context!, UnsafeMutablePointer<secp256k1_2_pubkey>(&publicKey), privateKeyPointer)
                return res
            } else {
                return nil
            }
    ...
    public static func serializePublicKey(publicKey: inout secp256k1_2_pubkey, compressed: Bool = false) -> Data? {
        ...
        let result = serializedPubkey.withUnsafeMutableBytes { (serializedPubkeyRawBuffPointer) -> Int32? in
            if let serializedPkRawPointer = serializedPubkeyRawBuffPointer.baseAddress, serializedPubkeyRawBuffPointer.count > 0 {
                ...
                return withUnsafeMutablePointer(to: &keyLength, { (keyPtr:UnsafeMutablePointer<Int>) -> Int32 in
                    withUnsafeMutablePointer(to: &publicKey, { (pubKeyPtr:UnsafeMutablePointer<secp256k1_2_pubkey>) -> Int32 in
                        let res = secp256k1_2_ec_pubkey_serialize(context!,
                                                                serializedPubkeyPointer,
                                                                keyPtr,
                                                                pubKeyPtr,
                                                                UInt32(compressed ? SECP256K1_2_EC_COMPRESSED : SECP256K1_2_EC_UNCOMPRESSED))
                        return res
                    })
                })
            } else {
    ...
    internal static func parsePublicKey(serializedKey: Data) -> secp256k1_2_pubkey? {
        ...
        let result = serializedKey.withUnsafeBytes { (serializedKeyRawBufferPointer: UnsafeRawBufferPointer) -> Int32? in
            if let serializedKeyRawPointer = serializedKeyRawBufferPointer.baseAddress, serializedKeyRawBufferPointer.count > 0 {
                ...
                let res = secp256k1_2_ec_pubkey_parse(context!, UnsafeMutablePointer<secp256k1_2_pubkey>(&publicKey), serializedKeyPointer, keyLen)
                return res
            } else {
                return nil
            }
    ...
    public static func parseSignature(signature: Data) -> secp256k1_2_ecdsa_recoverable_signature? {
        guard signature.count == 65 else {return nil}
        var recoverableSignature: secp256k1_2_ecdsa_recoverable_signature = secp256k1_2_ecdsa_recoverable_signature()
        ...
        let result = serializedSignature.withUnsafeBytes { (serRawBufferPtr: UnsafeRawBufferPointer) -> Int32? in
            if let serRawPtr = serRawBufferPtr.baseAddress, serRawBufferPtr.count > 0 {
                ...
                return withUnsafeMutablePointer(to: &recoverableSignature, { (signaturePointer:UnsafeMutablePointer<secp256k1_2_ecdsa_recoverable_signature>) -> Int32 in
                    let res = secp256k1_2_ecdsa_recoverable_signature_parse_compact(context!, signaturePointer, serPtr, v)
                    return res
                })
            } else {
                return nil
    ...
    internal static func serializeSignature(recoverableSignature: inout secp256k1_2_ecdsa_recoverable_signature) -> Data? {
            ...
            if let serSignatureRawPointer = serSignatureRawBufferPointer.baseAddress, serSignatureRawBufferPointer.count > 0 {
                ...
                return withUnsafePointer(to: &recoverableSignature) { (signaturePointer:UnsafePointer<secp256k1_2_ecdsa_recoverable_signature>) -> Int32 in
                    withUnsafeMutablePointer(to: &v, { (vPtr: UnsafeMutablePointer<Int32>) -> Int32 in
                        let res = secp256k1_2_ecdsa_recoverable_signature_serialize_compact(context!, serSignaturePointer, vPtr, signaturePointer)
                        return res
                    })
                }
            } else {
    ...
    internal static func recoverableSign(hash: Data, privateKey: Data, useExtraEntropy: Bool = false) -> secp256k1_2_ecdsa_recoverable_signature? {
                    ...
                    if let privateKeyRPointer = privateKeyRBPointer.baseAddress, privateKeyRBPointer.count > 0 {
                        ...
                        return extraEntropy.withUnsafeBytes({ (extraEntropyRBPointer) -> Int32? in
                            if let extraEntropyRPointer = extraEntropyRBPointer.baseAddress, extraEntropyRBPointer.count > 0 {
                                ...
                                return withUnsafeMutablePointer(to: &recoverableSignature, { (recSignaturePtr: UnsafeMutablePointer<secp256k1_2_ecdsa_recoverable_signature>) -> Int32 in
                                    let res = secp256k1_2_ecdsa_sign_recoverable(context!, recSignaturePtr, hashPointer, privateKeyPointer, nil, useExtraEntropy ? extraEntropyPointer : nil)
                                    return res
                                })
                            } else {
                                return nil
    ...
    public static func verifyPrivateKey(privateKey: Data) -> Bool {
        ...
        let result = privateKey.withUnsafeBytes { (privateKeyRBPointer) -> Int32? in
            if let privateKeyRPointer = privateKeyRBPointer.baseAddress, privateKeyRBPointer.count > 0 {
                let privateKeyPointer = privateKeyRPointer.assumingMemoryBound(to: UInt8.self)
                let res = secp256k1_2_ec_seckey_verify(context!, privateKeyPointer)
                return res
            } else {
                return nil
            }
